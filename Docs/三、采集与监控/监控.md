...
#### 数值类型统一
- **所有数值字段使用数字类型**，便于计算和排序
- **价格自动转换**：美元价格转换为人民币数值（汇率7.0）
- **单位处理**：移除所有单位后缀（"件"、"人关注"、"个商品"等）

#### 字段类型说明
- `goodsSold`: 数字类型 - 商品销量
- `goodsPromoPrice`: 数字类型 - 商品价格（人民币）
- `goodsRating`: 数字类型 - 商品评分
- `goodsReviews`: 数字类型 - 商品评价数
- `storeSold`: 数字类型 - 店铺总销量
- `storeFollowers`: 数字类型 - 店铺关注数
- `storeItemsNum`: 数字类型 - 店铺商品数量
- `storeRating`: 数字类型 - 店铺评分
...
#### 数据类型 (已更新)
- **数值格式**: 所有数值字段使用数字类型，便于计算和排序
- **单位处理**: 移除所有单位后缀，保留纯数值
- **价格格式**: 价格转换为人民币数值（如 13.23）

### 数据字段说明 (已更新)

#### goodsData（商品数据）
- `goodsSold`: 商品销量（数字类型，如 1054）
- `goodsPromoPrice`: 商品促销价格（数字类型，人民币，如 13.23）
- `goodsTitle`: 商品标题（字符串类型）
- `goodsRating`: 商品评分（数字类型，如 4.5）
- `goodsReviews`: 商品评价数量（数字类型，如 128）

#### storeData（店铺数据）
- `storeSold`: 店铺总销量（数字类型，如 600000）
- `storeFollowers`: 店铺关注者数量（数字类型，如 1019）
- `storeItemsNum`: 店铺商品数量（数字类型，如 53）
- `storeRating`: 店铺评分（数字类型，如 4.6）
- `storeName`: 店铺名称（字符串类型）
